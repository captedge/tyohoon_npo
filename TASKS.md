...
- [ ] 海上気象データソースの追加、①沿岸波浪予報（JMA地方海上予報＝VPCY51）から着手（2026-07-29着手）。表示範囲南側（フィリピン近海、北緯5〜19°）は地方海上予報区の対象外のため非表示のままとする方針を確認済み（2026-07-29）。予報区ポリゴンデータ・区域名定数・電文パーサー（`lib/utils/marine_areas.dart`／`marine_area_codes.dart`／`jma_marine_xml_parser.dart`）は実装済み——経緯・詳細は`docs/completed-log.md`と`docs/devlog-online-xml.md`参照。**次にやること**：
  ...
  - [ ] Open-Meteo波の場（波高）オーバーレイ：試用ダイアログ・航路周辺グリッド表示とも実装済み・実機確認済み（2026-07-29、`lib/utils/build_flags.dart`／`open_meteo_marine_fetcher.dart`／`wave_field.dart`新規、`map_painter.dart`／`map_screen.dart`に追加、`run_windows_personal.bat`／`build_release_personal.bat`新規、本流ビルドには影響なし）。ユーザーが実機で確認し「本当に良い、船長として役立つ」と高評価、**個人用として採用の方向で確定**（2026-07-29）。**次のステップ（未着手）**：表示範囲を「航路周辺のみ」から「地図の現在の表示範囲（画面に映っている分だけ）」方式に変更——ユーザーからの「エリア全体に展開したい」という要望に対し、固定の全表示範囲（N5-50/E85-170）を一括取得する方式は①データ量（格子点2万件超）②描画の重さ③粗い格子での見た目劣化、の3点でデメリットが大きいことを説明し、パン・ズームで見えている範囲だけを都度取得する方式（③案）を採用することで合意（2026-07-29、経緯・検討した3案は`docs/devlog-online-xml.md`「波の場オーバーレイの表示範囲方式」参照）。実装未着手（次回セッション着手）。波の色分け（0-4mを4段階、気象庁公式階級とは別の独自配色）についてもユーザーから質問があり回答済み——変更希望があれば別途対応。
