...
class VoyagePlanEntry {
  ...
  /// Used to find which registered plan(s) to cascade-delete when the same
  /// file is deleted from the library via "Edit CSV" (2026-07-27 request:
  /// "Editで削除：Passage Planに残っている場合は...削除して良いですか Y/N、
  /// 残っていない場合は質問なしで削除"), and to find which registered
  /// plan(s) to keep in sync when the same file is *renamed* via "Edit CSV"
  /// (2026-08-04 request — see `_MapScreenState._renameCsvLibraryEntry` in
  /// map_screen.dart). Kept in sync with the library filename on rename
  /// (reversing the original 2026-07-27 decision to leave this "orphaned"
  /// on rename — the user found the resulting stale name/menu/legend label
  /// confusing once they actually used the feature).
