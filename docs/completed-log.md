...
**船・台風アイコンを画像化**（2026-07-27）— プレースホルダーの三角形（船）・赤丸（台風）を、`assets/ship_icon01.png`／`assets/typhoon_icon01.png`の実画像に差し替え。台風アイコンは画像中心を座標に、船アイコンは左右中央・下から15%の位置を座標に（`_shipIconAnchorYFrac=0.85`）アンカーする方式で実装。船アイコンは既存の進行方向（次WP）回転ロジックをそのまま流用（画像の船首が上向きの絵のため、従来の三角形と同じ「angle=0で上向き」の前提と一致）。表示サイズはズームに対して固定（既存の`1/zoom`補正パターンを踏襲、船アイコン高さ26px・台風アイコン直径24px、従来のプレースホルダーに近いサイズ）。画像は`dart:ui`の`instantiateImageCodec`で`map_screen.dart`の`initState`で1回だけ読み込み（`lib/utils/marker_icons.dart`新設）、読み込み完了までは従来のプレースホルダー形状にフォールバックする設計。`pubspec.yaml`のassetsに両PNGを追加。確認済み：`map_painter.dart`・`map_screen.dart`・`marker_icons.dart`ともコメント除く括弧・波括弧・角括弧の対応が0で一致。Coworkサンドボックス（Linux）のためビルド未実施——Windows機での`flutter run -d windows`での見た目確認（アイコンの向き・アンカー位置・ズーム時のサイズ固定）が必要。

**Playback speedメニューを再生バー内へ移動**（2026-07-27）— AppBar上段にあった速度調整アイコン（`Icons.speed`）を撤去し、左下の再生バー内、再生／一時停止ボタンの真上に縦積みで配置。列の幅（52px）・再生バー自体の幅は変更せず、両ボタンとも`padding: EdgeInsets.zero`＋`constraints`縮小・アイコンサイズ縮小（速度18px、再生28px、従来34px）で既存の88px高の枠内に収めた。確認済み：`map_screen.dart`の括弧・波括弧・角括弧の対応が0で一致。Coworkサンドボックス（Linux）のためビルド未実施——Windows機での見た目・タップしやすさの確認が必要。

**Playback speedの調整範囲を1%〜100%に変更**（2026-07-27）— 従来25%〜150%だったスライダー範囲を、ユーザー要望により1%〜100%へ変更（`_minPlaybackSpeed`/`_maxPlaybackSpeed`）。刻み幅も新しい整数%範囲に合わせ5%刻みから1%刻みに変更。デフォルト値（50%）は新範囲内のためそのまま維持。確認済み：括弧・波括弧・角括弧の対応が0で一致。Windows機での見た目・動作確認が必要。

**メニューポップアップの表示名変更**（2026-07-27）— ①「Playback speed」→「Play Sp'd」（ダイアログタイトル・再生バー内アイコンのtooltip両方）、②台風の距離同心円メニューのtooltip「100/200nm rings」→「Range Ring」、③「Voyage plan」→「Passage Plan」（AppBarメニューのtooltip、`VoyagePlanScreen`のタイトル、メニュー項目「Edit voyage plan...」→「Edit passage plan...」、CSVファイル選択ダイアログのタイトルまで統一）。コード内部の変数・クラス名（`VoyagePlanScreen`等）はユーザー非表示のため変更対象外。確認済み：`map_screen.dart`・`voyage_plan_screen.dart`とも括弧・波括弧・角括弧の対応が0で一致。Windows機での見た目確認が必要。

**船・台風の名前メニューの表示名変更**（2026-07-27）— 「Ship's Name / Typhoon label」（tooltip）・「Ship's Name / Typhoons」（ダイアログタイトル）を「Information」に変更（当初「Infromation」と指定されたがスペルミスと判明し即修正）。ダイアログ内の各入力欄の見出し（"Ship's Name"、"Typhoon 1/2/3"）はメニュー名自体ではないため変更対象外。確認済み：括弧・波括弧・角括弧の対応が0で一致。Windows機での見た目確認が必要。

**アプリ起動時のデフォルト設定を変更**（2026-07-27）— ①Range Ring（台風の100/200nm距離同心円）のデフォルトをOffからOnに変更（`_TyphoonSlot.ringsEnabled`初期値）。②InformationメニューのDisplayトグルのデフォルトを、Ship・Typhoon 1のみOn、Typhoon 2/3はOffに変更（`_typhoonSlots`の`List.generate`で`i == 0`のときのみ`displayEnabled = true`とするよう変更。Shipは元々デフォルトOnのため変更なし）。確認済み：括弧・波括弧・角括弧の対応が0で一致。Windows機での見た目確認が必要。
