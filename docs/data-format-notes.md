...
- **公式サイト（`https://open-meteo.com/en/pricing`）記載のレート制限（無料／非商用枠、2026-07-30確認）**：
  ...
  - 日次：10,000コール
  - 月次：300,000コール
  - 「1コール」は基本的にHTTPリクエスト1回に対応（変数数・期間が極端に多い場合は按分カウントされる旨の記載あり、地点数による按分の明記は無いが、上記のバッチ分割により実際のHTTPリクエスト数が地点数に比例して増える設計になっている点に注意）。
  ...
  - **`Hourly`制限にも到達（2026-07-30実機確認）**：Minutely制限への到達→1分待って再試行、を開発中のテストで繰り返した結果、エラーメッセージが「Hourly」の制限超過に切り替わった（時間あたり5,000コールの枠も消費）。Minutelyは1分待てば復帰するが、Hourlyは文字通り最大1時間待つ必要がある。**開発・テスト時の教訓**：範囲を絞ってバッチ数を減らす対策と合わせて、同一セッション内でのテストの繰り返し方（短時間に何度も再試行しない等）にも注意が必要。取得範囲の絞り込み方針は検討中（TASKS.md参照）。
