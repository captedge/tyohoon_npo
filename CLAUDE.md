...
- **git運用**：`git init`実行、`user.name`/`user.email`をグローバル設定済み（`Capt.Edge` / `captain.edge.management@gmail.com`）、リモート`https://github.com/captedge/tyohoon_npo.git`を`origin`として追加済み（2026-07-25）。直近のpush：2026-07-27、`main`ブランチ、commit `2d69493`（JTWC警報の認識拡大・台風予報点の時刻ラベル・月またぎ日付バグ修正・配布用Zip作成）。本セッション分（Information/Passage Planのサンプル表示廃止・再生バー非表示化・CSVライブラリ機能一式・Import CSV件数表示の行移動）は未コミット、セッション終了時にコミット予定。詳細は `docs/operation-rules.md` のgit運用ルールに従う（commit/pushはユーザーが「コミットして」と発言したときのみ、`commit.bat`経由）。
