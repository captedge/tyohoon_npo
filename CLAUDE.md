...
- **船・台風の軌跡表示**：通過済み区間は実線、予定区間は点線（境界は現在時刻の補間点、2026-08-03確定）。台風アイコンは`typhoon_icon01.png`（中心アンカー）。船アイコンは`ship_01`〜`10.png`の色別10種（2026-07-31差替え、`kShipColorPalette`と同順。旧`ship_icon01.png`＋`ColorFilter`着色から画像切替へ変更）、左右中央・余白込み下限から25%上方アンカー（旧15%、新画像は余白大のため変更、詳細`docs/devlog-2026-07-31-ui-polish-and-wave-field-design.md`）。各アイコン・ラベル・WPドット・線の太さ・点線パターンは、地図をズームしても画面上で常に一定の見た目になるよう`MapPainter`が`zoom`を受け取り補正する設計（実装方法・往復の教訓は`docs/devlog-map-zoom-rendering.md`参照）。
