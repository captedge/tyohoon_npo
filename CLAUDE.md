...
- **git運用**：`git init`実行、`user.name`/`user.email`をグローバル設定済み（`Capt.Edge` / `captain.edge.management@gmail.com`）、リモート`https://github.com/captedge/tyohoon_npo.git`を`origin`として追加済み（2026-07-25）。直近のpush：2026-07-27、`main`ブランチ、commit `8200cbc`（登録情報の永続化・起動時デフォルト設定・JST日時表示位置変更）。今回のセッション分（月またぎ修正・再生バー消失防止・予想地点時刻ラベル・JTWC認識拡大・build_release.bat）は未コミット、セッション終了時にコミット予定。詳細は `docs/operation-rules.md` のgit運用ルールに従う（commit/pushはユーザーが「コミットして」と発言したときのみ、`commit.bat`経由）。
