...
- **地図UI**：ズームはピンチ/ホイール＋＋／－ボタン＋スライダー（下限統一済み、`docs/devlog-wheel-zoom.md`）。表示は英語表記。再生バーはWindy風、速度1〜100%調整可。カーソル緯度経度・船アイコン向きは`docs/devlog-map-overlays.md`参照。
- **メニュー構成**（2026-07-31時点）：AppBarは「Passage Plan」「Forecast」（旧称Information）の2つのみ。Range Ringは「Forecast」内で台風スロット・ソースごとにOn/Off。詳細（アイコン種別・Ship's Name移動先等）は`docs/devlog-2026-07-31-ui-polish-and-wave-field-design.md`参照。
