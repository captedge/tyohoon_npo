...
- **地図UI**：ズームはピンチ/ホイール＋＋／－ボタン＋スライダー、下限は3者で統一済み（詳細`docs/devlog-wheel-zoom.md`）。表示は英語表記。再生バーはWindy風（日付・時刻、速度25〜150%調整可）。カーソル緯度経度・船アイコン向き・距離表示（船の後方追従）は`TASKS.md`参照。東西表示範囲の拡張を検討中（`TASKS.md`「決定待ち・要確認事項」）。
- **船・台風のラベル**：船名はAppBarのMenuダイアログでユーザー入力。台風はJTWC警報テキスト貼り付けで番号・名称・気圧・位置・予報点を自動抽出、最大3つ登録可、船・台風とも個別にDisplay ON/OFF。再生開始時刻はTyphoon 1の発表時間（JST変換）。
