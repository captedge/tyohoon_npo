...
- **航海計画データ**：JRC ECDIS純正のルートCSVを、AppBarの船アイコンメニュー（Import CSV.../Edit voyage plan...）からインポート可能（2026-07-30実装済み）。パーサー・出発日時入力・WP追加/削除・区間速力編集は`VoyagePlanScreen`（`lib/screens/voyage_plan_screen.dart`）で実装済み。列構成・時刻計算ロジックは`docs/data-format-notes.md`参照。未インポート時は`_shipTrackSample`（サンプルデータ）にフォールバック。
...
- **船・台風の軌跡表示**：通過済み区間は実線、予定区間は点線（境界は現在時刻の補間点、2026-08-03確定）。船アイコン・台風アイコン・各ラベル・WPドット・線の太さ・点線パターンは、地図をズームしても画面上で常に一定の見た目になるよう`MapPainter`が`zoom`を受け取り補正する設計（実装方法・往復の教訓は`docs/devlog-map-zoom-rendering.md`参照）。
- **地図UI**：ズームはピンチ/ホイール＋＋／－ボタン＋スライダー、下限は3者で統一済み（詳細`docs/devlog-wheel-zoom.md`）。表示は英語表記。再生バーはWindy風（日付・時刻、速度25〜150%調整可）。カーソル緯度経度・船アイコン向きは`docs/devlog-map-overlays.md`、距離表示（船の後方追従）は`docs/completed-log.md`参照。
