...
- **台風データソース**：気象庁の「防災情報XML」（無認証・無料の電文フィード。正式なAPIではないため仕様変更リスクあり）とJTWC（米軍）テキスト（無料公開ページ）は、フォールバック関係ではなく**両方を同時に独立表示**する設計（2026-07-28確定）。台風スロットごとにJTWC/JMA双方をDisplay ON/OFF・色分け（JTWC=赤／JMA=オレンジ）・Range Ring（100/200nm）もソース別に独立ON/OFF可能（地図上の台風アイコンタップ、またはAppBarのRange Ringメニューから。メニューはソースごとに1項目、ラベルは地図と同じ「JTWC12W (NOUL)」形式）。台風ラベルは号数＋JMA公式の発達段階符号（気象図に表示されるもの、例TD/TS/STS）をソース名で接頭（`lib/utils/jma_xml_parser.dart`の`designation`/`classCode`参照）。WNIは有料APIのため個人利用では採用せず、画像は補助確認のみ。手動でのグリッド目視読み取りは誤差0.5〜2度（最大約30海里超）と大きく、通常データソースとしては不採用（判断根拠は `docs/devlog-map-design.md` 参照）。**JMA自動取得は`Information`ダイアログの「Fetch from JMA」ボタンで手動トリガー実装済み（2026-07-28、`lib/utils/jma_feed_fetcher.dart`）**：自動定期取得・オフラインキャッシュ・ON/OFF設定化は未実装（`docs/data-format-notes.md`・TASKS.md参照）。
...
- **今後の開発方針（仮・2026-07-28合意、変更可能性あり）**：オンライン化・スマホ対応とも同一プロジェクト・同一`main`ブランチで進め、複数の完成形を並行維持する運用は取らない。フェーズごとに一時的なフィーチャーブランチを切ってマージする通常運用とし、オンライン化（JMA XML自動取得）はアプリ内ON/OFF設定として実装（別バージョンにしない）、スマホ版UIはデスクトップ版と機能・デザインを極力共通化し横向き固定とする。進める順序は①オンライン化→②モバイル対応。判断の経緯・教訓は`docs/devlog-architecture-roadmap.md`参照。
